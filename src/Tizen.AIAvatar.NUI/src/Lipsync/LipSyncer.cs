...
namespace Tizen.AIAvatar
{
    internal class LipSyncer
    {
        ...
        protected void PlayLipAnimation()
        {
            ...
            {
                Log.Error(AIAvatar.LogTag, "Current Lip Animation is null");
            }
            lipAnimation?.Play();
        }
        ...
        private void PlayLipSync(byte[] audio)
        {
            if (audio == null)
            {
                Tizen.Log.Error(AIAvatar.LogTag, "audi data is null");
                return;
            }
            DestroyLipAnimation();

            var lipKeyframes = CreateKeyFrame(audio, AIAvatar.CurrentAudioOptions.SampleRate);
            if (lipKeyframes != null)
            {
                ...
                {
                    Tizen.Log.Error(AIAvatar.LogTag, "lipAnimation is null");
                }
            }
            else
            {
                Tizen.Log.Error(AIAvatar.LogTag, "lipKeyframes is null");
            }
        }
        ...
        private void PlayLipSync(string path)
        {
            ...
            {
                Log.Error(AIAvatar.LogTag, "Failed to load audio file");
            }
        }
        ...
        private Animation CreateLipAnimationByVowelsQueue(int sampleRate = 0)
        {
            if (sampleRate == 0)
            {
                sampleRate = AIAvatar.CurrentAudioOptions.SampleRate;
            }
            if (vowelPools.Count > 0)
            {
                AttachPreviousVowel(vowelPools.Dequeue(), out var newVowels);
                Log.Info(AIAvatar.LogTag, $"vowelRecognition: {String.Join(", ", newVowels)}");
        ...
        private AnimationKeyFrame CreateKeyFrame(byte[] audio, int sampleRate)
        {
            ...
            if (keyFrames == null)
            {
                Log.Error(AIAvatar.LogTag, $"Failed to initialize KeyFrames");
            }
